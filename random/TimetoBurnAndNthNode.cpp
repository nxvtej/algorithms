...
Node* buildTree(bool showPrompts) {
    int data;
    cin >> data;
    ...
    node->left = buildTree(showPrompts);

    if (showPrompts) {
        cout << "Enter right node data for " << node->data << " : ";
    }
    node->right = buildTree(showPrompts);
...
int main() {
    cout << "Would you like to enter the data all at once (y/n)? ";
    char choice;
    cin >> choice;

    bool singleInputMode = (choice == 'y' || choice == 'Y');
    if (!singleInputMode) {
        cout << "Step 1: Build tree\nEnter root data or -1 for nullptr: ";
    }
